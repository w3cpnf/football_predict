...
#comments
get_player_comment = tk.Label(root, text = 'Needs Club nbr from 1-18'+ '\n' + 'and Saison', font=('calibre', 8)) 
new_player_comment = tk.Label(root, text = 'Needs dataframe from clubs'+ '\n' + 'and Saison', font=('calibre', 8)) 
kader_comment = tk.Label(root, text = 'Needs dataframe from clubs'+ '\n' + 'season (yyyy/yy), transferfenster: 1,2', font=('calibre', 8)) 
player_worth_comment = tk.Label(root, text = 'Needs only dataframe from clubs', font=('calibre', 8)) 
get_url_comment = tk.Label(root, text = 'Needs Club nbr from 1-18'+ '\n' + 'saison, and transferwindow', font=('calibre', 8)) 
...
saison_label_2 = tk.Label(root, text = 'Kicker(2020-21)', font=('calibre', 10, 'bold'))
...
saison_label_2.place(relx = 0.8, rely = 0.12, relwidth = 0.2, relheight = 0.02)
saison_entry_2.place(relx = 0.85, rely = 0.15, relwidth = 0.1, relheight = 0.02)
...
def upload_new_player():
    db.upload_local_db_data(df_new_players, 6)
...
def upload_kader():
    db.upload_local_db_data(df_kader, 20)
    print("Data successfuly uploaded")
...
def get_ligainsider_url():
    ...
    df_url = l_url.get_url_via_google(saison, timewindow, club)
    print(df_url)
...
def get_ligainsider_players():
    ...
    saison = saison_entry.get()
    club = int(club_nbr_entry.get())
    df = ld.get_bundesliga_player_daten(saison, club)
    #add data to global dataframe
...
def get_data_ligainsider_players():
    ...
    saison = saison_entry.get()
    club = int(club_nbr_entry.get())
    df = ld.get_data_ligainsider(saison, club)
    #add data to global dataframe
...
def get_kicker_players():
    ...
    club = int(club_nbr_entry.get())
    saison_2 = saison_entry_2.get()
    timewindow = int(transferfenster_entry.get())
    #call class 
    f = kicker.kicker(spieltag, saison, saison_2, club, timewindow)
    df = f.get_kicker()
    #add data to global dataframe
...
def upload_perf_players():
    db.upload_local_db_data(df_perf_players, 10)
    print("Data successfuly uploaded")
...
#perf
button_get_perf_index = tk.Button(root, text = 'Get Perf Index',command = get_kicker_players)
button_get_perf_index.pack()
button_upload_perf_index = tk.Button(root, text = 'Upload Perf Index',command = upload_kicker_players)
...
button_upload_ligainsider_url = tk.Button(root, text = 'Upload Url Ligainsider',command = get_ligainsider_url)
