...
def get_opponent_statistic(vereins_id, gegner_id, saison, spieltag):
    ...
    df = df_ergebnisse_kategorisiert[df_ergebnisse_kategorisiert['Vereins_ID']==vereins_id]
    df = df[df['Gegner_ID']==gegner_id]

    df_before = df[df['Saison']<saison]
    df_saison = df[df['Saison']==saison]
    df_saison = df_saison[df_saison['Spieltag'] < spieltag]
    df_before_game = df_before.append(df_saison)
    win = len(df_before_game[df_before_game['Spiel_Ausgang'] == 1])
    loss = len(df_before_game[df_before_game['Spiel_Ausgang'] == -1])
    statistic = round((1 + win)/(1 + loss), 2)

    df_stat = pd.DataFrame({'Vereins_ID': [vereins_id], 'Gegner_ID': [gegner_id], 'Saison': [saison], 'Spieltag': [spieltag], 'Statistic_Feature': [statistic]})

    return df_stat

def opponent_statistic_all():
    
    df_ergebnisse_kategorisiert = db.get_table('bl1_data_ergebnisse_kategorisiert')
    df_ergebnisse_kategorisiert = df_ergebnisse_kategorisiert[df_ergebnisse_kategorisiert['Saison']>='2014/15']
    df_complete = pd.DataFrame()
    saisons = df_ergebnisse_kategorisiert['Saison'].drop_duplicates()

    for saison in saisons:
    
        df_saison = df_ergebnisse_kategorisiert[df_ergebnisse_kategorisiert['Saison']==saison]
          
        for spieltag in range(1,35):
            df_spieltag = df_saison[df_saison['Spieltag']==spieltag]
            
            for verein in df_spieltag['Vereins_ID'].drop_duplicates():
                df_verein = df_spieltag[df_spieltag['Vereins_ID']==verein]
                
                for gegner in df_verein['Gegner_ID'].drop_duplicates():                    
                    df = get_opponent_statistic(verein, gegner, saison, spieltag)
                    df_complete = df_complete.append(df)
                    
    return df_complete
                    
def opponent_statistic_saison_spieltag(saison, spieltag):
    ...
    df_complete = df_complete[df_complete['Saison'] == saison]
    df_complete = df_complete[df_complete['Spieltag'] == spieltag]
    return df_complete

#df = opponent_statistic_saison_spieltag('2021/22', 5)
#db.upload_local_data_to_database(df_complete, 'bl1_feature_opponent_statistic')
...
#e = get_results_class(30, '2021/22')
#df = e.get_results()
...
class get_premierleague_results_class:
    ...
    def get_results(self):
        ...
        df_clubs = df_clubs.iloc[0:20]
        print(df_clubs)
        #df_clubs = df_clubs[df_clubs['Verein']!='Wolverhampton Wanderers']
        #df_clubs = df_clubs[df_clubs['Verein']!='Manchester City']
        #df_clubs = df_clubs[df_clubs['Verein']!='Aston Villa']
...
#e = get_premierleague_results_class(34, '2021/22')
#df = e.get_results()
#df = df.iloc[0:6,:]
#df = df.dropna()
