...
def get_plan(df, spieltag, saison):
    ...
    #df_all = df_all[df_all['Vereins_ID']==17]
...
def get_plan_premier_league(df, spieltag, saison):
    ...
    for trai in trainer:
        ...
        for v in vereine:
            ...
            print(von)
            df_splan_verein = df_spielplan[df_spielplan['Vereins_ID']==v]
            print(df_splan_verein)
            print(df_splan_verein['Datum'])
            df_spam = df_splan_verein[df_splan_verein['Datum']>=von]
            df_spam = df_spam[df_spam['Datum']<=bis]
            print(df_spam)
            df_ready = df_spam.merge(df_vt, on = ['Vereins_ID', 'Verein'], how = 'inner')
...
#df = get_trainer_premier_league('2021/22')
#df_new = new_trainer(df)
#db.upload_local_data_to_database(df_new, 'master_trainer_id')
#df_plan = get_plan_premier_league(df, 1, '2021/22')

#db.upload_local_data_to_database(df_plan, 'pl_data_trainer_spiele')
